 ...
 * @LastEditTime: 2025-04-02 18:27:26
   ...
   public:
    ...
    bool init() {
        try {
            model_ = std::make_unique<model::Qwen2Model>(
                base::TokenizerType::EncodeBpe, tokenizer_path_, model_path_,
                false);
    ...
    std::string generate(const std::string& prompt, int max_length) {
        ...
        tensor::Tensor pos_tensor =
            model_->get_buffer(model::ModelBufferType::InputPos);
        ...
        while (pos < max_length) {
            ...
            if (pos < prompt_len - 1) {
                tensor::Tensor input =
                    model_->fill_input(pos_tensor, prompt_embedding, is_prompt);
                ...
                tensor::Tensor input =
                    model_->fill_input(pos_tensor, token_embedding, is_prompt);
                model_->predict(input, pos_tensor, is_prompt, next);
            }
            ...
            if (is_prompt) {
                ...
                // 检查是否生成了结束标记
                if (pos >= 3) {
                    auto decoded = model_->decode(
                        std::vector<int32_t>(words.end() - 4, words.end()));
        ...
        // 提取生成的回复，去除提示部分
        std::vector<int32_t> response_tokens(words.begin() + prompt_len,
                                             words.end());
...
int main(int argc, char* argv[]) {
    ...
    // 打印ChatML格式prompt的函数
    auto print_chatml_prompt =
        [&assistant](const std::vector<ChatMessage>& history) {

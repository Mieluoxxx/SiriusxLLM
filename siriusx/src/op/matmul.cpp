...
MatmulLayer::MatmulLayer(base::DeviceType device_type, int32_t dim0,
    ...
    : LayerParam(device_type, LayerType::LayerMatmul, is_quant_layer, "Matmul"),
      dim0_(dim0),
      dim1_(dim1),
      has_bias_(has_bias) {
    reset_input_size(1);
...
base::Status MatmulLayer::check() const {
    ...
    if (!is_quant_layer_) {
        ...
        status = check_tensor_with_dim(get_weight(0), device_type_,
                                       base::DataType::Int8, dim0_, dim1_);
        if (!status) {
            LOG(ERROR) << "The weight tensor error in the matmul layer.";
            return status;
        }
    ...
    if (is_quant_layer_) {
        status = check_tensor_with_dim(scales_, device_type_,
                                       base::DataType::FP32, scales_.size());
        if (!status) {
...
base::Status MatmulLayer::set_bias(int32_t idx, int32_t& dim,
    ...
    if (!is_quant_layer_) {
        ...
        // is quant layer
        tensor::Tensor bias(base::DataType::Int8, dim);
        bias.set_device_type(device_type);
        CHECK(bias.assign(buffer));
        bias_.at(idx) = bias;

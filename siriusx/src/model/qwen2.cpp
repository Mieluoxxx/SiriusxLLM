...
Qwen2Model::Qwen2Model(base::TokenizerType tokenizer_type, std::string token_path,
    ...
    : Model(tokenizer_type, base::ModelType::ModelTypeLLama2, std::move(token_path),
...
base::Status Qwen2Model::init(base::DeviceType device_type) {
  ...
  if (device_type_ == base::DeviceType::CPU) {
    ...
    kernel::sin_cos_cache_calc_cuda(config_->head_size_, config_->seq_len_,
                                  ...
                                  get_buffer(ModelBufferType::CosCache), cuda_config_->stream);
  }
...
void Qwen2Model::init_mem() {
  ...
  tensor::Tensor sin_cache(base::DataType::FP32, config_->head_size_ * config_->seq_len_,
                           true, alloc);
  tensor::Tensor cos_cache(base::DataType::FP32, config_->head_size_ * config_->seq_len_,
                           true, alloc);
  ...
  // kv cache
  tensor::Tensor key_cache(base::DataType::FP32, config_->layer_num_, config_->seq_len_,
                           config_->kv_dim_, true, alloc);
  tensor::Tensor value_cache(base::DataType::FP32, config_->layer_num_, config_->seq_len_,
                             config_->kv_dim_, true, alloc);
  ...
  // Attention output
  tensor::Tensor attn(base::DataType::FP32, config_->head_num_, config_->seq_len_, true,
                      alloc);
  ...
  if (device_type_ == base::DeviceType::CUDA) {
    tensor::Tensor forward_output_cpu(base::DataType::FP32, config_->vocab_size_, true,
                                      alloc_cpu);
...
void Qwen2Model::feed_forward(int32_t layer_idx, const tensor::Tensor& input) const {
  ...
  STATUS_CHECK(
      qwen_layers_->add_layer_->forward(input, get_buffer(ModelBufferType::AttnOut), input));

 ...
 * @Date: 2025-06-27 15:54:47
 * @LastEditors: Morgan Woods weiyiding0@gmail.com
 * @LastEditTime: 2025-06-27 15:56:11
...
void sin_cos_cache_calc_cpu(int head_size, int max_seq_len, float* sin_cache,
    ...
    for (int pos = 0; pos < max_seq_len; ++pos) {
        for (int head_dim = 0; head_dim < head_size; ++head_dim) {
            float freq =
                1.0f / std::pow(1000000.0f, static_cast<float>(head_dim) /
...
void rope_kernel_cpu(int32_t dim, int32_t kv_dim, int32_t head_size,
    ...
    for (int32_t i = 0; i < dim; i += head_size) {
        for (int32_t head_dim = i % head_size; head_dim < head_size / 2;
             head_dim++) {
            float fci =
                *(sin_cache.ptr<float>() + pos * head_size + head_dim * 2);
            float fcr =
                *(cos_cache.ptr<float>() + pos * head_size + head_dim * 2);
            ...
            for (int32_t v = 0; v < rotn; v++) {
                float* vec = const_cast<float*>(
                    v == 0 ? input_q.ptr<float>()
                           : input_k.ptr<float>());  // the vector to rotate
                                                     // (query or key)
...
void rope_kernel_cpu(int32_t dim, int32_t kv_dim, int32_t head_size,
    ...
    for (int32_t i = 0; i < dim; i += 2) {
        ...
        for (int32_t v = 0; v < rotn; v++) {
            float* vec = const_cast<float*>(
                v == 0 ? input_q.ptr<float>()
                       : input_k.ptr<float>());  // the vector to rotate (query
                                                 // or key)
            float v0 = vec[i];
            float v1 = vec[i + 1];
            vec[i] = v0 * fcr - v1 * fci;

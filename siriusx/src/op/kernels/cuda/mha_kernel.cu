...
__device__ void softmax_cuda(float* __restrict__ x, int size) {
    int tid = threadIdx.x;
    int step = blockDim.x;
...
__global__ void multi_head_attention_kernel(
    ...
    for (int t = threadIdx.x; t <= pos; t += blockDim.x) {
         ...
         *
         *  xq = xq.transpose(1, 2)  # 转置后形状为 (heads, sequence_length,
         * head_dim) # 如果sequence_length为1，则形状简化为 (heads, head_dim)
         *  keys = keys.transpose(1, 2)  # 同样转置keys，得到形状 (heads,
         * sequence_length, head_dim) # 若sequence_length为1，则形状也简化为
         * (heads, head_dim)
         *
...
#pragma unroll
        for (int i = 0; i < head_size; i += 4) {
            ...
            float4 query_head_float4 =
                *reinterpret_cast<float4*>(query_head + i);
...
#pragma unroll
        for (int t = 0; t <= pos; t++) {
            float* value_head =
                value_cache + layer_offset + t * kv_dim + head_offset;

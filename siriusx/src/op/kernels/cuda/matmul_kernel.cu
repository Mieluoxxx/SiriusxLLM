...
__global__ void matmul_kernel_cuda_fp32(const float* input, const float* weight, float* output, int M,
                                      int K) {
...
template <int THREAD_PER_BLOCK, int ROW_PER_BLOCK>
__global__ void matmul_kernel_cuda_fp32int8(const float* input, const int8_t* weight,
...
void matmul_kernel_cuda(const tensor::Tensor& input, const tensor::Tensor& weight,
  ...
  CHECK(weight.device_type() == base::DeviceType::CUDA);
  const int32_t K = weight.get_dim(0);  // row
  const int32_t M = weight.get_dim(1);  // col

  CHECK_EQ(M, input.get_dim(0));
  if (config && config->stream) {
    matmul_kernel_cuda_fp32<128, 1><<<K, 128, 0, config->stream>>>(
        input.ptr<float>(), weight.ptr<float>(), const_cast<float*>(output.ptr<float>()), M, K);
  } else {
    matmul_kernel_cuda_fp32<128, 1><<<K, 128>>>(input.ptr<float>(), weight.ptr<float>(),
                                              const_cast<float*>(output.ptr<float>()), M, K);
  }
}

void matmul_kernel_cuda_qint8(const tensor::Tensor& input, const tensor::Tensor& weight,
  ...
  CHECK_EQ(M, input.get_dim(0));
  if (config->stream) {
    matmul_kernel_cuda_fp32int8<128, 1><<<K, 128, 0, config->stream>>>(
        input.ptr<float>(), weight.ptr<int8_t>(), scale.ptr<float>(), group_size,
        const_cast<float*>(output.ptr<float>()), M, K);
  } else {
    matmul_kernel_cuda_fp32int8<128, 1><<<K, 128>>>(input.ptr<float>(), weight.ptr<int8_t>(),

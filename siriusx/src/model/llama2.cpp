...
void LLama2Model::create_param_layers() {
    ...
    llama2_layers_->embedding_layer_ = std::make_shared<op::EmbeddingLayer>(device_type_, config_->dim_, config_->seq_len_, std::abs(config_->vocab_size_));

    const void* weight_embedding = raw_model_data_->weight(0);
    llama2_layers_->embedding_layer_->set_weight(0, {std::abs(config_->vocab_size_), config_->dim_}, weight_embedding, cpu_device_type);

    auto create_matmul_layer = [&](int32_t rows, int32_t cols, size_t& pos) {
        auto w = std::make_shared<op::MatmulLayer>(device_type_, rows, cols);
        w->set_weight(0, {rows, cols}, this->raw_model_data_->weight(pos), cpu_device_type);
        pos += rows * cols;
        return w;
    };

    int32_t dim = config_->dim_;
    // pos表示当前权重在raw_model_data_中的位置
    size_t pos = dim * std::abs(config_->vocab_size_) + dim * config_->layer_num_;

    // 创建 q, k, v, o 的权重矩阵
    for(int32_t i = 0; i < config_->layer_num_; i++) {
        llama2_layers_->wq_layers_.push_back(create_matmul_layer(dim, dim, pos));
        llama2_layers_->wk_layers_.push_back(create_matmul_layer(config_->kv_dim_, dim, pos));
        llama2_layers_->wv_layers_.push_back(create_matmul_layer(config_->kv_dim_, dim, pos));
        llama2_layers_->wo_layers_.push_back(create_matmul_layer(dim, dim, pos));
    }
    ...
    for(int32_t i = 0; i < config_->layer_num_; i++) {
        llama2_layers_->w1_layers_.push_back(create_matmul_layer(hidden_dim, dim, pos));
        llama2_layers_->w2_layers_.push_back(create_matmul_layer(dim, hidden_dim, pos));
        llama2_layers_->w3_layers_.push_back(create_matmul_layer(hidden_dim, dim, pos));
    }
    ...
    // 创建 cls 层
    llama2_layers_->cls_layer_ = std::make_shared<op::MatmulLayer>(device_type_, config_->vocab_size_, dim);
    // 共享权重
    if (config_->is_shared_weight_) {
        llama2_layers_->cls_layer_->set_weight(0, {config_->vocab_size_, dim}, this->raw_model_data_->weight(0), cpu_device_type);
    }
    else {
        llama2_layers_->cls_layer_->set_weight(0, {config_->vocab_size_, dim}, this->raw_model_data_->weight(pos), cpu_device_type);
    }

    // 创建 rmsnorm 层
    auto create_rmsnorm_layer = [&](size_t& pos) {
        auto layer = std::make_shared<op::RMSNormLayer>(device_type_, config_->dim_);
        const void* weight_rmsnorm = raw_model_data_->weight(pos);
        layer->set_weight(0, {config_->dim_}, weight_rmsnorm, cpu_device_type);
        pos += config_->dim_;
        return layer;
    };
    ...
    for (int32_t i = 0; i < config_->layer_num_; ++i) {
        llama2_layers_->rmsnorm_layers_.push_back(create_rmsnorm_layer(rmsnorm_pos));
    }
...
base::Status LLama2Model::create_layers() {
    ...
    auto check_matmul_layers = [&](const std::vector<std::shared_ptr<op::Layer>>& layers, const std::string& layer_name){
        if(layers.size() != config_->layer_num_) {
            return error::InternalError("Failed to create the correct number of " + layer_name + " layers.");
        }
        for (const auto& layer : layers) {
            if (!layer) {
                return error::InternalError("Failed to create a " + layer_name + " layer.");
            }
        }
        return error::Success();
    };

    auto status = check_matmul_layers(llama2_layers_->wq_layers_, "WQ");
    if (!status.get_err_code()) return status;
    status = check_matmul_layers(llama2_layers_->wk_layers_, "WK");
    if (!status.get_err_code()) return status;
    status = check_matmul_layers(llama2_layers_->wv_layers_, "WV");
    if (!status.get_err_code()) return status;
    status = check_matmul_layers(llama2_layers_->wo_layers_, "WO");
    if (!status.get_err_code()) return status;
 
    // 检查 FFN 层的 matmul 层是否创建成功
    status = check_matmul_layers(llama2_layers_->w1_layers_, "W1");
    if (!status.get_err_code()) return status;
    status = check_matmul_layers(llama2_layers_->w2_layers_, "W2");
    if (!status.get_err_code()) return status;
    status = check_matmul_layers(llama2_layers_->w3_layers_, "W3");
    if (!status.get_err_code()) return status;

    // 检查其他关键层是否创建成功
    auto check_layer = [&](const std::shared_ptr<void>& layer, const std::string& layer_name) {
        if (!layer) {
            return error::InternalError("Failed to create the " + layer_name + " layer.");
        }
        return error::Success();
    };

    status = check_layer(llama2_layers_->rope_layer_, "RoPE");
    if (!status.get_err_code()) return status;
    status = check_layer(llama2_layers_->add_layer_, "Add");
    if (!status.get_err_code()) return status;
    status = check_layer(llama2_layers_->mha_layer_, "MHA");
    if (!status.get_err_code()) return status;
    status = check_layer(llama2_layers_->swiglu_layer_, "SwiGLU");
    if (!status.get_err_code()) return status;
...
op::EmbeddingOutput LLama2Model::embedding(const std::vector<int>& tokens) const {
    ...
    auto input_token_num = tensor::Tensor(base::DataType::Int32, static_cast<int32_t>(tokens.size()));
    LOG_IF(FATAL, !llama2_layers_->embedding_layer_) << "The embedding layer in the llama2 model is null pointer.";
    STATUS_CHECK(llama2_layers_->embedding_layer_->forward(input_tokens, input_token_num, input_embeddings));
...
void LLama2Model::attention_qkv(int32_t layer_idx, const tensor::Tensor& pos_tensor) const {
    ...
    // rope
    CHECK_NE(llama2_layers_->rope_layer_, nullptr) << "The RoPE layer in the attention block is null pointer.";
...
void LLama2Model::feed_forward(int32_t layer_idx, const tensor::Tensor& input) const {
    ...
    STATUS_CHECK(llama2_layers_->add_layer_->forward(
    ...
    const auto& ffn_rmsnorm =
        llama2_layers_->rmsnorm_layers_.at(layer_idx + config_->layer_num_);
    ...
    STATUS_CHECK(
        llama2_layers_->swiglu_layer_->forward(w1_output, w3_ouput, w1_output));
...
void LLama2Model::cls_logits(const tensor::Tensor& input) const {
    CHECK(llama2_layers_ != nullptr);
    const auto& norm =
        llama2_layers_->rmsnorm_layers_.at(2 * config_->layer_num_);

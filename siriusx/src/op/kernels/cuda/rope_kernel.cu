...
__global__ void rope_kernel_cuda_fp32(int pos, int dim, int kv_dim,
    ...
    for (int v = 0; v < rotn; v++) {
        float* vec = const_cast<float*>(
            v == 0 ? input_q : input_k);  // the vector to rotate (query or key)
        float v0 = vec[v0_idx];
        float v1 = vec[v1_idx];
...
__global__ void sin_cos_calc(int head_size, int max_seq_len, float* sin_cache,
    ...
    for (int pos = 0; pos < max_seq_len; ++pos) {
        float freq = 1.0f / pow(500000.0f, static_cast<float>(head_dim) /
...
__global__ void sin_cos_calc(int head_size, int max_seq_len, float* sin_cache,
    ...
    int head_dim = idx % head_size;
    for (int pos = 0; pos < max_seq_len; ++pos) {
        float freq = 1.0f / pow(1000000.0f, static_cast<float>(head_dim) /
                                                static_cast<float>(head_size));
        float val = static_cast<float>(pos) * freq;
        float fcr = cosf(val);
        float fci = sinf(val);

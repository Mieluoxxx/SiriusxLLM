...
void LLama2Model::create_nonparam_layers() {
    ...
    llama2_layers_->rope_layer_ = std::make_shared<op::RoPELayer>(
        device_type_, config_->dim_, config_->kv_dim_, config_->head_size_);
    llama2_layers_->mha_layer_ = std::make_shared<op::MultiHeadAttention>(
        device_type_, 0, config_->kv_mul_, config_->kv_dim_, config_->seq_len_,
        config_->head_num_, config_->head_size_);
    llama2_layers_->add_layer_ =
        std::make_shared<op::VecAddLayer>(device_type_);
    llama2_layers_->swiglu_layer_ =
...
void LLama2Model::create_param_quant_layers() {
    ...
    llama2_layers_->cls_layer_ = cls_layer;
    ...
    llama2_layers_->embedding_layer_->set_weight(
...
void LLama2Model::create_param_layers() {
    ...
    llama2_layers_->cls_layer_ = std::make_shared<op::MatmulLayer>(
    ...
    if (config_->is_shared_weight_) {
        ...
        llama2_layers_->cls_layer_->set_weight(
            0, {config_->vocab_size_, dim}, this->raw_model_data_->weight(pos),
            cpu_device_type);
    }
...
base::Status LLama2Model::create_layers() {
    ...
    for (int32_t i = 0; i < config_->layer_num_; ++i) {
        if (!llama2_layers_->wq_layers_.at(i) ||
            !llama2_layers_->wk_layers_.at(i) ||
            !llama2_layers_->wv_layers_.at(i) ||
            !llama2_layers_->wo_layers_.at(i)) {
            return error::InternalError(
    ...
    for (int32_t i = 0; i < config_->layer_num_; ++i) {
        if (!llama2_layers_->w1_layers_.at(i) ||
            !llama2_layers_->w2_layers_.at(i) ||
...
op::EmbeddingOutput LLama2Model::embedding(
    ...
    STATUS_CHECK(llama2_layers_->embedding_layer_->forward(
...
void LLama2Model::attention_rms(int32_t layer_idx,
    ...
    std::shared_ptr<op::Layer> rmsnorm_layer =
        llama2_layers_->rmsnorm_layers_.at(layer_idx);
    if (!rmsnorm_layer) {
